            ...
            <tbody className="divide-y divide-gray-800">
              {products.map((p) => (
                <tr key={p._id} className="hover:bg-gray-800/50 transition-colors">
                  ...
                  <td className="px-4 py-3 text-white font-medium">KSh {p.price.toLocaleString()}</td>
                  <td className="px-4 py-3">
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-gray-900 border border-gray-700 rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSave} className="p-6 space-y-4">
              {[['name', 'Product Name'], ['category', 'Category'], ['brand', 'Brand']].map(([key, label]) => (
                <div key={key}>
                  ...
                  <input value={form[key]} onChange={(e) => setForm({ ...form, [key]: e.target.value })}
                    required={key !== 'brand'}
                    className="w-full bg-gray-800 border border-gray-700 rounded-xl px-3 py-2.5 text-white text-sm focus:outline-none focus:border-primary-500" />
                </div>
              ))}
              <div>
                ...
                <textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })}
                  required rows={3}
                  className="w-full bg-gray-800 border border-gray-700 rounded-xl px-3 py-2.5 text-white text-sm focus:outline-none focus:border-primary-500 resize-none" />
              </div>
              ...
              <div>
                <label className="text-xs text-gray-400 mb-1 block">Image URLs (comma-separated)</label>
              ...
              <div>
                <label className="text-xs text-gray-400 mb-1 block">Upload Images</label>
                <label className="inline-flex items-center gap-2 px-3 py-2 rounded-xl border border-gray-700 text-sm text-gray-300 hover:bg-gray-800 cursor-pointer">
                  ...
                  <div className="grid grid-cols-4 gap-2 mt-3">
                    {form.images.map((img) => (
                      <div key={img} className="relative group">
                        <img src={img} alt="Product" className="w-full h-14 object-cover rounded-lg border border-gray-700" />
                        <button
                          type="button"
              ...
              <div className="flex gap-3 pt-2">
                <button type="button" onClick={() => setShowModal(false)} className="btn-secondary flex-1 border-gray-700 text-gray-300">Cancel</button>
