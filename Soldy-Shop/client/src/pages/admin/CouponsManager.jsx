            ...
            <tbody className="divide-y divide-gray-800">
              {coupons.map((c) => (
                <tr key={c._id} className="hover:bg-gray-800/40">
                  ...
                  <td className="px-4 py-3 text-gray-400">{c.usedCount}{c.usageLimit ? `/${c.usageLimit}` : ''}</td>
                  <td className="px-4 py-3 text-gray-400 text-xs">
                    {c.expiresAt ? new Date(c.expiresAt).toLocaleDateString() : '—'}
                  </td>
                  <td className="px-4 py-3">
                    <button onClick={() => handleToggle(c._id, c.isActive)}
                      className={`badge cursor-pointer ${c.isActive ? 'bg-green-900 text-green-400' : 'bg-gray-800 text-gray-500'}`}>
            ...
            <form onSubmit={handleSave} className="p-6 space-y-4">
              <div>
                ...
                <input value={form.code} onChange={(e) => setForm({ ...form, code: e.target.value.toUpperCase() })}
                  ...
                  className="w-full bg-gray-800 border border-gray-700 rounded-xl px-3 py-2.5 text-white text-sm focus:outline-none focus:border-primary-500 font-mono uppercase" />
              </div>
              <div>
              ...
              {form.type !== 'free-shipping' && (
                <div>
                  <label className="text-xs text-gray-400 mb-1 block">Value ({form.type === 'percentage' ? '%' : 'KSh'})</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs text-gray-400 mb-1 block">Min Order (KSh)</label>
